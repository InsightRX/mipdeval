test_that("calculate_shrinkage returns correct structure", {
  ...
  result <- calculate_shrinkage(list(results = res_df, mod_obj = mod_obj))
...
test_that("calc_shrinkage helper function works correctly", {
  ...
  expected_shrinkage <- round(100 * (1 - (sd(eta_values) / sqrt(0.09))), 1)
...
test_that("calculate_shrinkage validates input structure", {
  ...
  expect_error(
    calculate_shrinkage(list(results = bad_res_df, mod_obj = mod_obj))
  )
})
